...
export default function Header() {
  ...
  return (
    <>
      ...
      <aside
        className={clsx(
          'fixed top-0 left-0 bottom-0 z-50 bg-[#F1F5FA] border-r border-gray-200 shadow-2xl',
          'w-16 lg:w-20', 
          'transition-none'
        )}
      >
          ...
          >
            <div className="flex flex-col items-center gap-1">
              ...
              <svg
                ...
                className="flex-shrink-0 transition-all duration-300 group-hover:scale-110 group-hover:drop-shadow-lg"
              >
                <path
              ...
              {/* Myway 텍스트 */}
              <h1 className="text-xl font-bold text-[#007DE4] tracking-tight transition-all duration-300 group-hover:text-[#0066c0] group-hover:-translate-y-0.5">
                Myway
              </h1>
            </div>
          </div>
              ...
              {mainMenu.map((item) => {
                ...
                return (
                  <a
                    key={item.name}
                    href={item.href}
                    className="group flex flex-col items-center gap-2 py-3 rounded-2xl transition-all duration-200 hover:scale-105"
                  >
                    <Icon36
                      name={item.icon}
                      className={clsx(
                        'flex-shrink-0 transition-colors duration-300',
                        'w-8 h-8 lg:w-9 lg:h-9',           // 모바일 32px → 데스크톱 36px
                    ...
                    <span
                      className={clsx(
                        ...
                        'text-[11px] lg:text-xs',           // 모바일 11px → 데스크톱 12px
                        isActive ? 'text-[#007DE4]' : 'text-[#04152F]'
                      )}
                    >
                      {item.name}
