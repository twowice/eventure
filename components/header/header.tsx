...
import { mainmenu, type panelkey } from './headermenu';
...
export default function header({ children }: headerprops) {
  ...
  useEffect(() => {
    ...
    setopenpanel(current ? current.panelkey : null);
  }, [pathname]);

  const handlemenuclick = (href: string, panelkey: panelkey) => {
  ...
  const currenttitle = mainmenu.find(m => m.panelkey === openpanel)?.name || '메뉴';
  ...
  return (
    <>
      ...
      <aside className="fixed inset-y-0 left-0 z-50 w-16 lg:w-20 bg-[#F1F5FA] border-r border-gray-200 shadow-2xl">
        <div className="flex flex-col h-full">
          ...
          >
            <div className="flex flex-col items-center gap-1">
              <svg width="22" height="17" viewBox="0 0 20 14" fill="none" xmlns="http://www.w3.org/2000/svg"
                ...
                <path d="M3.49609 12.3945L3.23926 13.1416C3.10036 13.5448 2.7194 13.8161 2.29297 13.8164H1.00293C0.30008 13.816 -0.184127 13.1093 0.0703125 12.4541L1.79785 8.00781L3.49609 12.3945ZM19.7822 12.4531C20.0377 13.1085 19.5539 13.8159 18.8506 13.8164H17.5439C17.1171 13.816 16.7372 13.5444 16.5986 13.1406L16.3457 12.4023L18.0469 8.00586L19.7822 12.4531ZM3.5625 3.48535H9.21289 10.6309 16.2842L17.5088 6.62598L15.8428 10.9326L14.2598 6.30957C13.9565 5.4249 12.7133 5.40373 12.3799 6.27734L12.0957 7.02344C12.0101 7.2485 12.008 7.49753 12.0908 7.72363L13.8252 12.4531C14.0638 13.1052 13.581 13.7961 12.8867 13.7969H12.085 11.268 11.1338 10.8691 10.5691 9.26795 8.96777 8.70215 8.56805 8.18428 7.75195 6.96094 6.26876 5.78598 6.02051 7.73047 7.8103 7.80935 7.72754 7.46387 7.13737 5.886 5.58008 4 2.33496 3.56055 3.5625Z" fill="#0077D4"/>
              </svg>
              ...
              {mainmenu.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={clsx(
                      'w-full flex flex-col items-center gap-2 py-3 rounded-2xl transition-all duration-300 hover:scale-105',
          ...
          <div className="p-2 border-t border-gray-200 shrink-0">
            <button className="w-full py-3 text-center text-base font-medium text-[#04152F] hover:text-[#007DE4] transition-colors">
              로그인
            </button>
          </div>
        </div>
      ...
      {/* 슬라이드 패널 */}
      <SlidePanel isopen={!!openpanel} onclose={() => setopenpanel(null)} title={currenttitle}>
        {children}
      </SlidePanel>
    </>
  );
